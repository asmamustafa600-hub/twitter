...
train_lstm.py
--------------
Trains an LSTM model for tweet sentiment classification and saves
...
def main():
    ...
    print(f"\nLSTM validation accuracy: {accuracy:.4f}")

    model.save(f"{MODEL_DIR}/lstm_model.keras")
    joblib.dump(tokenizer, f"{MODEL_DIR}/lstm_tokenizer.joblib")

    with open(f"{MODEL_DIR}/lstm_accuracy.json", "w") as f:
        json.dump({"model": "LSTM", "accuracy": accuracy}, f)
