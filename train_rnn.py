...
train_rnn.py
------------
Trains a SimpleRNN model for tweet sentiment classification and saves
...
def main():
    ...
    print(f"\nSimpleRNN validation accuracy: {accuracy:.4f}")

    model.save(f"{MODEL_DIR}/rnn_model.keras")
    joblib.dump(tokenizer, f"{MODEL_DIR}/rnn_tokenizer.joblib")

    with open(f"{MODEL_DIR}/rnn_accuracy.json", "w") as f:
        json.dump({"model": "SimpleRNN", "accuracy": accuracy}, f)
